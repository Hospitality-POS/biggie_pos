  ...
  <>
    <style>{`
      @media print {
        ...
        #sales-thermal, #sales-thermal * { visibility: visible; }
        #sales-thermal { position: absolute; inset: 0; }
      }
    `}</style>
    <div id="sales-thermal" ref={ref}
      ...
      <div style={{ textAlign: "center", marginBottom: 10 }}>
        ...
        <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: "1.5px" }}>SALES REPORT</div>
  ...
  <>
    <style>{`
      @media print {
        ...
        body * { visibility: hidden; }
        #sales-a4, #sales-a4 * { visibility: visible; }
        #sales-a4 { position: absolute; inset: 0; }
      }
    `}</style>
    <div id="sales-a4" ref={ref}
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            <div style={{ fontSize: 13, fontWeight: 600, opacity: 0.85, letterSpacing: "1px" }}>SALES REPORT</div>
          </div>
      ...
      <div style={{ display: "flex", background: C.bg, borderBottom: `2px solid ${C.border}`, padding: "8px 8px 8px 0" }}>
        {[
          ...
          { label: "Categories", value: `${data.length}`, color: C.blue },
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
          <thead>
            <tr style={{
              ...
              <th style={{ padding: "9px 12px", textAlign: "right", fontWeight: 700, width: 80 }}>Qty</th>
              <th style={{ padding: "9px 12px", textAlign: "right", fontWeight: 700, width: 120 }}>Total (KES)</th>
            </tr>
          </thead>
          <tbody>
            {data.map((item: any, i: number) => {
              ...
              return (
                <React.Fragment key={item.id || i}>
                  ...
                  <tr style={{
                    ...
                    <td colSpan={2} style={{ padding: "8px 12px", fontWeight: 700, fontSize: 12, color: C.primary }}>{item.name || "Uncategorized"}</td>
...
const SalesReportModal: React.FC<SalesReportProps> = ({ openM, onCloseM, startDate, endDate }) => {
  ...
  const { salesReport: rawData, loading } = useAppSelector((state) => state.Report);
  ...
  return (
      ...
      title={
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingRight: 32 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <Text strong style={{ fontSize: 14, color: C.darkText }}>Sales Report</Text>
          </div>
          <Segmented
            value={printMode}
    ...
    >
      <Spin spinning={loading} tip="Generating sales report…" style={{ display: "block", width: "100%" }}>
        {!hasData && !loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {hasData && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                <SummaryCard label="Overall Total" value={`KES ${fmt(overallTotal)}`} color={C.primary} bg={C.primaryLight} icon={<BarChartOutlined />} />
                <SummaryCard label="Categories" value={`${data.length}`} color={C.blue} bg="#eff6ff" icon={<ShoppingOutlined />} />
              </div>
            )}
