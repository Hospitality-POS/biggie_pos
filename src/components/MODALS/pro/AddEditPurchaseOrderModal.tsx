...
const AddEditPurchaseOrderModal: React.FC<AddEditPurchaseOrderModalProps> = ({
    ...
    const handleFinish = async (values: any) => {
        ...
        if (confirmed) {
            try {
                const processedItems = values.po_items?.map((item: any) => ({
            ...
            >
                <StepsForm.StepForm
                    ...
                    onFinish={async (values) => {
                        // Store step 1 values
                        setStepFormValues(prev => ({ ...prev, ...values }));
                        return true;
                    }}
                    initialValues={
                        edit
                                    ...
                                    <Card title="Order Items" style={{ marginBottom: 16 }}>
                                        <Space direction="vertical" style={{ width: '100%' }}>
                                            {(() => {
                                                ...
                                                const validItems = itemsValue.filter((item: any) =>
                                                                    ...
                                                                    <Text strong>
                                                                        {(() => {
                                                                            ...
                                                                            if (typeof inventoryValue === 'object' && inventoryValue?.label) {
                                                                                return inventoryValue.label.split(' (Last:')[0]; // Remove price hint
                                                                            }
