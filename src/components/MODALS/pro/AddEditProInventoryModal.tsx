/**
 * AddEditProInventoryModal.tsx
 *
 * Inventory add/edit modal with integrated barcode scanning:
...
const BarcodeScannerModal: React.FC<BarcodeScannerModalProps> = ({ open, onClose, onScan }) => {
  ...
  // Use navigator.mediaDevices directly (enumerateDevices was removed in @zxing/library v0.19+)
  useEffect(() => {
    if (!open) return;
    setScanned(null); setError(null); setLoading(true);

    // Camera requires HTTPS — block early with a clear message on plain HTTP
    if (location.protocol !== "https:" && location.hostname !== "localhost" && location.hostname !== "127.0.0.1") {
      setError("Camera access requires HTTPS. Open the app via https:// on your network. See console for setup instructions.");
      console.error("[BarcodeScanner] Camera blocked: app must be served over HTTPS on non-localhost origins. Run: npm install -D vite-plugin-mkcert and add mkcert() to your vite.config.ts plugins.");
      setLoading(false);
      return;
    }
    ...
    navigator.mediaDevices.enumerateDevices()
      ...
      .catch(() => {
        // enumerateDevices may return empty labels without a prior getUserMedia call
        // Try requesting camera access first then enumerate
  ...
  useEffect(() => {
    ...
    setScanning(true);
    // decodeFromVideoDevice still works in all versions
  ...
  const handleConfirm = () => {
    if (scanned) { onScan(scanned); handleClose(); }
  };

  const handleClose = () => {
    stopScanner(); setScanned(null); setError(null); onClose();
  };
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {scanned ? (
          <>
            ...
            <Button type="primary" onClick={handleConfirm} style={{ flex: 2, borderRadius: 8, height: 38, background: "#059669", border: "none", fontWeight: 600 }}>
              Use This Barcode
            </Button>
          </>
        ) : (
...
const useBarcodeScanner = ({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      // Ignore if user is typing in an input/textarea — don't intercept normal typing
...
const AddEditProInventoryModal: React.FC<AddInventoryDialogProps> = ({ data, actionRef, edit }) => {
  ...
  // ── USB/Bluetooth scanner — only active when this modal is open ────────────
  useBarcodeScanner({
    enabled: open,
    onScan: useCallback((code: string) => {
      form.setFieldValue("barcode", code);
      // Navigate to step 2 where the barcode field is, if not already there
      if (currentStep !== 2) {
  ...
  const handleFormSubmit = async (values: any) => {
    try {
      saveCurrentStepData();
      const finalValues = { ...getAllFormData(), ...values };
  ...
  useEffect(() => {
    if (open && edit && data) {
      ...
      setStepData({
        0: { name: fv.name, usage_type: fv.usage_type, category: fv.category, subcategory_id: fv.subcategory_id, unit_id: fv.unit_id, desc: fv.desc },
                ...
                <Space.Compact style={{ width: "100%" }}>
                  <Form.Item name="barcode" noStyle>
                    <input
                      ...
                      style={{ flex: 1, height: 36, border: "1px solid #d9d9d9", borderRight: "none", borderRadius: "6px 0 0 6px", padding: "0 11px", fontSize: 14, outline: "none", width: "calc(100% - 84px)" }}
                  ...
                  <Tooltip title="Scan with camera">
                    <Button
                      ...
                      style={{ height: 36, borderRadius: "0 6px 6px 0", background: "#eef2ff", border: "1px solid #d9d9d9", color: "#6366f1", fontWeight: 500, padding: "0 10px" }}
                    >
                      Scan
                    </Button>
                  </Tooltip>
