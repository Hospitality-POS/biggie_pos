    ...
    if (file.status === "done" || file.originFileObj || file instanceof File) {
      try {
        ...
        if (!allowedTypes.includes(uploadFile.type)) {
          message.error(
            "Only image files are allowed (JPEG, PNG, GIF, SVG, WebP)"
          );
          setFileList([]);
          return;
        }
  ...
  if (!tenantId) {
    return (
      <PageContainer
        title={
          <>
            <SettingOutlined /> Tenant Settings
          </>
        }
                ...
                >
                  <Space>
                    <Tag
                      ...
                      icon={getStatusIcon(tenant.subscription_status)}
                    >
                      {tenant.subscription_status
                        .replace("_", " ")
                        .toUpperCase()}
                    </Tag>
                    <Tag color="blue">{tenant.subscription_cycle}</Tag>
                    {tenant.subscription_id && (
                      <Tag color="purple">
                        {typeof tenant.subscription_id === "object"
                          ? `${tenant.subscription_id.name} - ksh ${tenant.subscription_id.price}`
                          : "Subscription"}
                      </Tag>
                    )}
                  </Space>
                  <Text>
                    ...
                    {tenant.business_type_name ||
                      (typeof tenant.business_type === "object"
