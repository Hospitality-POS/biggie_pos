...
export const TrialBalanceTable: React.FC<{ data: TrialBalanceResponse }> = ({ data }) => {
    const columns = [
        {
            title: "Code", dataIndex: "account_code", key: "code", width: 90,
        ...
        },
        {
            ...
            render: (v: string, r: TrialBalanceRow) => (
                <Space size={4}>
                    <Text>{v}</Text>
                    <Tag color={TYPE_COLORS[r.account_type]} style={{ fontSize: 10, padding: "0 4px" }}>
                        {r.account_type}
                    </Tag>
                </Space>
            ),
        ...
        {
            title: "Subtype", dataIndex: "account_subtype", key: "subtype", width: 160,
            render: (v: string) => <Text type="secondary" style={{ fontSize: 12 }}>{v || "—"}</Text>
        },
        {
            title: "Opening Balance", dataIndex: "opening_balance", key: "ob", width: 130, align: "right" as const,
            render: (v: number) => <Text style={{ fontSize: 12 }}>{fmt(v)}</Text>
        },
        {
            title: "Period Debit", dataIndex: "period_debit", key: "pd", width: 120, align: "right" as const,
            render: (v: number) => v > 0 ? <Text style={{ color: "#cf1322" }}>{fmt(v)}</Text> : "—"
        },
        {
            title: "Period Credit", dataIndex: "period_credit", key: "pc", width: 120, align: "right" as const,
            render: (v: number) => v > 0 ? <Text style={{ color: "#389e0d" }}>{fmt(v)}</Text> : "—"
        },
        {
            title: "Closing Debit", dataIndex: "closing_debit", key: "cd", width: 120, align: "right" as const,
            render: (v: number) => v > 0 ? <Text strong style={{ color: "#cf1322" }}>{fmt(v)}</Text> : "—"
        },
        {
            title: "Closing Credit", dataIndex: "closing_credit", key: "cc", width: 120, align: "right" as const,
            render: (v: number) => v > 0 ? <Text strong style={{ color: "#389e0d" }}>{fmt(v)}</Text> : "—"
        },
    ];
            ...
            <Row gutter={16} style={{ marginBottom: 16 }}>
                <Col span={6}>
                    <Card size="small" bordered>
                        <Statistic title="Total Debit" value={data.totals.total_debit} precision={2} prefix="KES"
                            valueStyle={{ color: "#cf1322", fontSize: 16 }} />
                    </Card>
                </Col>
                <Col span={6}>
                    <Card size="small" bordered>
                        <Statistic title="Total Credit" value={data.totals.total_credit} precision={2} prefix="KES"
                            valueStyle={{ color: "#389e0d", fontSize: 16 }} />
                    </Card>
                </Col>
                <Col span={6}>
                    <Card size="small" bordered
                        style={{ background: data.is_balanced ? "#f6ffed" : "#fff2f0", borderColor: data.is_balanced ? "#b7eb8f" : "#ffa39e" }}>
                        <Statistic title="Difference" value={Math.abs(data.totals.difference)} precision={2} prefix="KES"
                            valueStyle={{ color: data.is_balanced ? "#389e0d" : "#cf1322", fontSize: 16 }}
                            suffix={data.is_balanced ? <CheckCircleOutlined /> : <WarningOutlined />} />
                    </Card>
                </Col>
                <Col span={6}>
                    <Card size="small" bordered>
                        <div style={{ paddingTop: 4 }}>
                            ...
                            <div style={{ marginTop: 8 }}>
                                <Badge
                                    status={data.is_balanced ? "success" : "error"}
                                    text={<Text strong style={{ fontSize: 15 }}>{data.is_balanced ? "Balanced ✓" : "Unbalanced ✗"}</Text>}
                                />
                            </div>
                        </div>
                    </Card>
                    ...
                    <Table.Summary fixed>
                        <Table.Summary.Row style={{ background: "#fafafa", fontWeight: 700 }}>
                            ...
                            <Table.Summary.Cell index={4} align="right">
                                <Text strong style={{ color: "#cf1322" }}>{fmt(data.totals.total_debit)}</Text>
                            </Table.Summary.Cell>
                            <Table.Summary.Cell index={5} align="right">
                                <Text strong style={{ color: "#389e0d" }}>{fmt(data.totals.total_credit)}</Text>
                            </Table.Summary.Cell>
    ...
    return (
        <>
            <Row gutter={16} style={{ marginBottom: 20 }}>
                <Col span={8}>
                    <Card size="small" bordered style={{ background: "#f6ffed", borderColor: "#b7eb8f" }}>
                        <Statistic title="Total Revenue" value={data.revenue.total_revenue} precision={2} prefix="KES"
                            valueStyle={{ color: "#389e0d", fontSize: 18 }} suffix={<ArrowUpOutlined />} />
                    </Card>
                </Col>
                <Col span={8}>
                    <Card size="small" bordered style={{ background: "#fff2f0", borderColor: "#ffa39e" }}>
                        <Statistic title="Total Expenses" value={data.expenses.total_expenses} precision={2} prefix="KES"
                            valueStyle={{ color: "#cf1322", fontSize: 18 }} suffix={<ArrowDownOutlined />} />
                    </Card>
                </Col>
                <Col span={8}>
                    <Card size="small" bordered
                        style={{ background: data.is_profit ? "#f6ffed" : "#fff2f0", borderColor: data.is_profit ? "#b7eb8f" : "#ffa39e" }}>
                        <Statistic
                            title={data.is_profit ? "Net Profit" : "Net Loss"}
                            value={Math.abs(data.net_profit)}
                            precision={2}
                            prefix="KES"
                            valueStyle={{ color: data.is_profit ? "#389e0d" : "#cf1322", fontSize: 20, fontWeight: 700 }}
                        />
                    </Card>
                </Col>
            </Row>
            ...
            <Table
                ...
                columns={[
                    ...
                    {
                        title: "Subtype", dataIndex: "account_subtype", width: 160,
                        render: (v: string) => <Text type="secondary" style={{ fontSize: 12 }}>{v || "—"}</Text>
                    },
                    {
                        title: "Amount", dataIndex: "amount", align: "right" as const, width: 150,
                        render: (v: number) => <Text strong style={{ color: "#389e0d" }}>{fmt(v)}</Text>
                    },
                ]}
                summary={() => (
                    <Table.Summary.Row style={{ background: "#f6ffed" }}>
                        <Table.Summary.Cell index={0} colSpan={3}><Text strong>Total Revenue</Text></Table.Summary.Cell>
                        <Table.Summary.Cell index={3} align="right">
                            <Text strong style={{ color: "#389e0d", fontSize: 14 }}>{fmt(data.revenue.total_revenue)}</Text>
                        </Table.Summary.Cell>
                    </Table.Summary.Row>
                )}
            />
            ...
            <Table
                ...
                summary={() => (
                    <Table.Summary.Row style={{ background: "#fff2f0" }}>
                        <Table.Summary.Cell index={0} colSpan={3}><Text strong>Total Expenses</Text></Table.Summary.Cell>
                        <Table.Summary.Cell index={3} align="right">
                            <Text strong style={{ color: "#cf1322", fontSize: 14 }}>{fmt(data.expenses.total_expenses)}</Text>
                        </Table.Summary.Cell>
                    </Table.Summary.Row>
                )}
            />
...
export const BalanceSheetTable: React.FC<{ data: BalanceSheetResponse }> = ({ data }) => {
    const section = (
        title: string,
        rows: { account_code: string; account_name: string; account_subtype?: string; balance: number }[],
        total: number,
        color: string
    ) => (
        <div style={{ marginBottom: 24 }}>
            ...
            <Table
                ...
                columns={[
                    ...
                    {
                        title: "Balance", dataIndex: "balance", align: "right" as const, width: 140,
                        render: (v: number) => <Text style={{ color }}>{fmt(v)}</Text>
                    },
                ]}
                summary={() => (
                    <Table.Summary.Row style={{ background: "#fafafa" }}>
                        <Table.Summary.Cell index={0} colSpan={3}><Text strong>Total {title}</Text></Table.Summary.Cell>
                        <Table.Summary.Cell index={3} align="right">
                            <Text strong style={{ color, fontSize: 14 }}>{fmt(total)}</Text>
                        </Table.Summary.Cell>
                    </Table.Summary.Row>
                )}
            />
    ...
    return (
        <>
            <Alert
                ...
                showIcon
                message={data.is_balanced
                    ? `Balance Sheet is balanced as of ${dayjs(data.as_of).format("DD MMM YYYY")}`
                    : `Balance Sheet is UNBALANCED — difference: KES ${fmt(data.totals.difference)}`}
                style={{ marginBottom: 20 }}
            />
            <Row gutter={16} style={{ marginBottom: 20 }}>
                <Col span={8}>
                    <Card size="small" bordered><Statistic title="Total Assets" value={data.totals.total_assets} precision={2} prefix="KES" valueStyle={{ color: "#1d39c4", fontSize: 16 }} /></Card>
                </Col>
                <Col span={8}>
                    <Card size="small" bordered><Statistic title="Liabilities + Equity" value={data.totals.total_liabilities_and_equity} precision={2} prefix="KES" valueStyle={{ color: "#1d39c4", fontSize: 16 }} /></Card>
                </Col>
                <Col span={8}>
                    <Card size="small" bordered style={{ background: data.is_balanced ? "#f6ffed" : "#fff2f0" }}>
                        <Statistic title="Difference" value={Math.abs(data.totals.difference)} precision={2} prefix="KES" valueStyle={{ color: data.is_balanced ? "#389e0d" : "#cf1322", fontSize: 16 }} />
                    </Card>
                </Col>
            </Row>
                ...
                <div key={acc.account_code} style={{ marginBottom: 32 }}>
                    <Space style={{ marginBottom: 8 }} wrap>
                        ...
                        <Text type="secondary" style={{ fontSize: 12 }}>
                            Opening: KES {fmt(acc.opening_balance)} | Closing: KES {fmt(acc.closing_balance)} |
                            DR: {fmt(acc.total_debit)} | CR: {fmt(acc.total_credit)}
                        </Text>
                    </Space>
                    <Table
...
export const VATReportTable: React.FC<{ data: VATReportResponse }> = ({ data }) => {
    ...
    const sourceRows = Object.entries(by_source).map(([src, vals]) => ({
        source: src,
        collected: vals.collected,
        paid: vals.paid,
        net: vals.collected - vals.paid,
    }));
            ...
            <Row gutter={16} style={{ marginBottom: 20 }}>
                <Col span={6}>
                    <Card size="small" bordered style={{ background: "#f6ffed" }}>
                        <Statistic title="VAT Collected (Output)" value={summary.vat_collected} precision={2} prefix="KES" valueStyle={{ color: "#389e0d", fontSize: 16 }} />
                    </Card>
                </Col>
                <Col span={6}>
                    <Card size="small" bordered style={{ background: "#fff7e6" }}>
                        <Statistic title="VAT Paid (Input)" value={summary.vat_paid} precision={2} prefix="KES" valueStyle={{ color: "#fa8c16", fontSize: 16 }} />
                    </Card>
                </Col>
                <Col span={6}>
                    <Card size="small" bordered style={{ background: summary.net_vat_payable >= 0 ? "#fff2f0" : "#f6ffed" }}>
                        <Statistic title="Net VAT Payable" value={Math.abs(summary.net_vat_payable)} precision={2} prefix="KES"
                            valueStyle={{ color: summary.net_vat_payable >= 0 ? "#cf1322" : "#389e0d", fontSize: 16 }}
                            suffix={summary.net_vat_payable < 0 ? "(Refund)" : ""} />
                    </Card>
                </Col>
                <Col span={6}>
                    <Card size="small" bordered>
                        <Text type="secondary" style={{ fontSize: 12, display: "block" }}>VAT Account</Text>
                        <Text code style={{ fontSize: 14 }}>{summary.vat_account_code}</Text>
                    </Card>
                </Col>
            </Row>
...
export const CashFlowTable: React.FC<{ data: CashFlowResponse }> = ({ data }) => (
    <>
        <Row gutter={16} style={{ marginBottom: 20 }}>
            <Col span={8}>
                <Card size="small" bordered style={{ background: "#f6ffed" }}>
                    <Statistic title="Total Inflows" value={data.totals.total_inflows} precision={2} prefix="KES"
                        valueStyle={{ color: "#389e0d", fontSize: 16 }} suffix={<ArrowUpOutlined />} />
                </Card>
            </Col>
            <Col span={8}>
                <Card size="small" bordered style={{ background: "#fff2f0" }}>
                    <Statistic title="Total Outflows" value={data.totals.total_outflows} precision={2} prefix="KES"
                        valueStyle={{ color: "#cf1322", fontSize: 16 }} suffix={<ArrowDownOutlined />} />
                </Card>
            </Col>
            <Col span={8}>
                <Card size="small" bordered style={{ background: data.totals.net_cash_flow >= 0 ? "#f6ffed" : "#fff2f0" }}>
                    <Statistic title="Net Cash Flow" value={Math.abs(data.totals.net_cash_flow)} precision={2} prefix="KES"
                        valueStyle={{ color: data.totals.net_cash_flow >= 0 ? "#389e0d" : "#cf1322", fontSize: 16 }}
                        suffix={data.totals.net_cash_flow < 0 ? "(Outflow)" : "(Inflow)"} />
                </Card>
            </Col>
        </Row>
        <Table
            ...
            columns={[
                ...
                {
                    title: "Net Cash Flow", dataIndex: "net_cash_flow", width: 130, align: "right" as const,
                    render: (v: number) => <Text strong style={{ color: v >= 0 ? "#389e0d" : "#cf1322" }}>{fmt(Math.abs(v))}</Text>
                },
                {
                    title: "Closing", dataIndex: "closing_balance", width: 120, align: "right" as const,
                    render: (v: number) => <Text strong style={{ color: "#1d39c4" }}>{fmt(v)}</Text>
                },
            ]}
        />
    </>
    ...
    <>
        <Row gutter={12} style={{ marginBottom: 20 }}>
            {Object.entries(data.summary).map(([key, val]) => (
                <Col span={4} key={key}>
                    <Card size="small" bordered>
                        <Statistic title={key.replace("total_", "").toUpperCase()} value={parseFloat(val as string) || 0} precision={2} prefix="KES"
                            valueStyle={{ fontSize: 13, color: TYPE_COLORS[key.replace("total_", "").toUpperCase()] || "#333" }} />
                    </Card>
                </Col>
            ))}
        </Row>
...
export const CustomerStatementTable: React.FC<{ data: CustomerStatementResponse }> = ({ data }) => {
    ...
    const TYPE_COLORS_TXN: Record<string, string> = {
        Invoice: "blue", Payment: "green", "Credit Note": "cyan", Bill: "orange", "Debit Note": "red",
    };
        ...
        <>
            <Row gutter={16} style={{ marginBottom: 16 }}>
                ...
                <Col span={12}>
                    <Row gutter={8}>
                        ...
                        <Col span={8}>
                            <Card size="small" bordered style={{ background: summary.closing_balance > 0 ? "#fff2f0" : "#f6ffed" }}>
                                <Statistic title={summary.balance_label} value={Math.abs(summary.closing_balance)} precision={2} prefix="KES"
                                    valueStyle={{ fontSize: 13, color: summary.closing_balance > 0 ? "#cf1322" : "#389e0d", fontWeight: 700 }} />
                            </Card>
                        </Col>
                    </Row>
                </Col>
            ...
            <Table
                ...
                columns={[
                    ...
                    { title: "Type", dataIndex: "type", width: 110, render: (v: string) => <Tag color={TYPE_COLORS_TXN[v] || "default"} style={{ fontSize: 10 }}>{v}</Tag> },
            ...
            <Row gutter={16} style={{ marginBottom: 16 }}>
                ...
                <Col span={12}>
                    <Row gutter={8}>
                        ...
                        <Col span={8}>
                            <Card size="small" bordered style={{ background: summary.closing_balance > 0 ? "#fff7e6" : "#f6ffed" }}>
                                <Statistic title={summary.balance_label} value={Math.abs(summary.closing_balance)} precision={2} prefix="KES"
                                    valueStyle={{ fontSize: 13, color: summary.closing_balance > 0 ? "#fa8c16" : "#389e0d", fontWeight: 700 }} />
                            </Card>
                        </Col>
                    </Row>
                </Col>
    ...
    return (
        <>
            <Row gutter={16} style={{ marginBottom: 16 }}>
                <Col span={8}>
                    <Card size="small" bordered style={{ background: "#fff2f0" }}>
                        <Statistic title="Total AR Outstanding" value={grandTotal} precision={2} prefix="KES"
                            valueStyle={{ color: "#cf1322", fontSize: 18, fontWeight: 700 }} />
                    </Card>
                </Col>
                <Col span={8}>
                    <Card size="small" bordered>
                        <Statistic title="Customers with Outstanding" value={data.total_customers_with_outstanding}
                            valueStyle={{ fontSize: 20 }} />
                    </Card>
                </Col>
                <Col span={8}>
                    <Card size="small" bordered>
                        <Text type="secondary" style={{ fontSize: 12, display: "block" }}>As of</Text>
                        <Text strong style={{ fontSize: 16 }}>{dayjs(data.as_of).format("DD MMM YYYY")}</Text>
                    </Card>
                </Col>
            </Row>
                ...
                <div key={cust.customer_id} style={{ marginBottom: 24 }}>
                    ...
                    <Table
                        ...
                        columns={[
                            ...
                            {
                                title: "Days Overdue", dataIndex: "days_overdue", width: 110, align: "right" as const,
                                render: (v: number) => <Tag color={v > 90 ? "red" : v > 30 ? "orange" : "default"}>{v}d</Tag>
                            },
    ...
    return (
        <>
            <Row gutter={16} style={{ marginBottom: 16 }}>
                <Col span={8}>
                    <Card size="small" bordered style={{ background: "#fff7e6" }}>
                        <Statistic title="Total AP Outstanding" value={grandTotal} precision={2} prefix="KES"
                            valueStyle={{ color: "#fa8c16", fontSize: 18, fontWeight: 700 }} />
                    </Card>
                </Col>
                <Col span={8}>
                    <Card size="small" bordered>
                        <Statistic title="Suppliers with Outstanding" value={data.total_suppliers_with_outstanding}
                            valueStyle={{ fontSize: 20 }} />
                    </Card>
                </Col>
                <Col span={8}>
