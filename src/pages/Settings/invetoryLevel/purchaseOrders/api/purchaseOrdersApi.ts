...
export const editPurchaseOrder = async (params: ParamsType) => {
  try {
    console.log("Editing purchase order:", params)
    const response = await axiosInstance.put(`${url}/${params?._id}`, {
      ...params.values,
      supplier_id:
        params.values.supplier_id?.value || params.values.supplier_id,
...
export const createDeliveryFromPO = async (params: ParamsType) => {
  try {
    const response = await axiosInstance.post(
      `${url}/${params.purchase_order_id}/delivery`,
      {
...
export const getDeliveriesByPurchaseOrder = async (
  purchase_order_id: string
) => {
  try {
    const response = await axiosInstance.get(
      `${BASE_URL}/deliveries/purchase-order/${purchase_order_id}`
    );
    return response.data;
  } catch (error) {
    throw new Error(error);
