...
import {
  downloadCustomerTemplate, analyseCustomerFile, importCustomersFromExcel, AnalyseCustomerResult, ImportCustomerResult,
    ...
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
      <thead>
        <tr style={{ background: "#f8fafc" }}>
          ...
          <th style={{ padding: "8px 10px", textAlign: "left", borderBottom: "1px solid #e2e8f0", color: "#64748b", fontWeight: 500 }}>Customer Field</th>
          <th style={{ padding: "8px 10px", textAlign: "left", borderBottom: "1px solid #e2e8f0", color: "#64748b", fontWeight: 500 }}>Status</th>
        </tr>
      </thead>
      <tbody>
        {Object.entries(mapping).map(([excelCol, customerField]) => {
          const isMissing = missingRequired.includes(customerField) || missingRecommended.includes(customerField);
          return (
            <tr key={excelCol} style={{ borderBottom: "1px solid #f1f5f9" }}>
              <td style={{ padding: "6px 10px", color: "#334155" }}>{excelCol}</td>
              <td style={{ padding: "6px 10px", color: "#334155" }}>{customerField}</td>
...
const ImportCustomersModal: React.FC<ImportCustomersModalProps> = ({ onSuccess }) => {
  const [open, setOpen] = useState(false);
  const [file, setFile] = useState<File | null>(null);
  const [step, setStep] = useState<"upload" | "analysing" | "advice" | "importing" | "result">("upload");
  const [analysis, setAnalysis] = useState<AnalyseCustomerResult | null>(null);
  const [importResult, setImportResult] = useState<ImportCustomerResult | null>(null);
  ...
  const handleAnalyse = async (selectedFile: File) => {
    setStep("analysing");
    try {
      const result = await analyseCustomerFile(selectedFile);
      setAnalysis(result);
      setStep("advice");
    } catch (err: any) {
  ...
  const handleImport = async () => {
    ...
    setStep("importing");
    try {
      const result = await importCustomersFromExcel(file, shopId, assignedTo, rowAssignments);
  ...
  const renderUpload = () => (
    <div>
      ...
      <Text strong style={{ fontSize: 13, color: "#374151", display: "block", marginBottom: 6 }}>Upload any Excel file with customer data</Text>
      ...
      <div>
        ...
        <div style={{ marginBottom: 14, padding: "12px", background: "#f0f9ff", borderRadius: 8, border: "1px solid #bae6fd" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            ...
            <Text strong style={{ fontSize: 13, color: "#0369a1" }}>Assign customers to staff (optional)</Text>
          </div>
          <Text style={{ fontSize: 11, color: "#64748b", display: "block", marginBottom: 8 }}>
            Optionally assign all imported customers to a staff member, or assign individually in the preview table below.
          </Text>
          <Select
        ...
        <div style={{ display: "flex", gap: 8, paddingTop: 4 }}>
          ...
          >
            {analysis.canImport
              ? `Import ${analysis.totalDataRows} Customer${analysis.totalDataRows !== 1 ? "s" : ""}`
  ...
  const renderImporting = () => (
    <div style={{ textAlign: "center", padding: "40px 0" }}>
      <Spin size="large" />
      <Text style={{ display: "block", marginTop: 16, color: "#64748b", fontSize: 14 }}>Importing customers…</Text>
      <Text style={{ display: "block", marginTop: 6, color: "#94a3b8", fontSize: 12 }}>
        {assignedTo ? "Assigning to selected staff member and creating customers" : "Creating customers"}
      </Text>
    </div>
  );
        ...
        {assignedTo && (
          <div style={{ background: "#f0fdf4", borderRadius: 8, padding: "8px 12px", marginBottom: 12, border: "1px solid #bbf7d0" }}>
            <Text style={{ fontSize: 12, color: "#065f46" }}>
              ...
              All customers assigned to selected staff member
            </Text>
          </div>
        )}
